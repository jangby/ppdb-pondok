...
$classes = ($active ?? false)
            ? 'flex items-center gap-3 px-3 py-2.5 rounded-lg bg-blue-600 text-white shadow-md transition-all group relative'
            : 'flex items-center gap-3 px-3 py-2.5 rounded-lg text-slate-300 hover:bg-slate-800 hover:text-white transition-all group relative';
...
$icons = [
    ...
    'dormitory' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />',
];
@endphp
...
<a {{ $attributes->merge(['class' => $classes]) }}>
    ...
    <span :class="sidebarExpanded ? 'opacity-100' : 'opacity-0 hidden w-0'" 
          class="whitespace-nowrap transition-all duration-300 origin-left">
        {{ $slot }}
    </span>

    {{-- Tooltip --}}
    <div :class="sidebarExpanded ? 'hidden' : 'group-hover:block hidden'" 
         class="absolute left-14 bg-gray-900 text-white text-xs px-2 py-1 rounded shadow-lg z-50 whitespace-nowrap">
        {{ $slot }}
    </div>
</a>