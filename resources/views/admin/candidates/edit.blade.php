    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-bold text-2xl text-gray-800 leading-tight">
                Edit Data: {{ $candidate->nama_lengkap }}
            </h2>
            <a href="{{ route('admin.candidates.show', $candidate->id) }}" class="text-gray-500 hover:text-gray-700 font-medium text-sm flex items-center gap-1">
                ...
                <div class="bg-white shadow-md rounded-xl overflow-hidden mb-6 border border-gray-100">
                    ...
                    <div class="p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        <div class="lg:col-span-2">
                            ...
                            <input type="text" name="nama_lengkap" value="{{ old('nama_lengkap', $candidate->nama_lengkap) }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <input type="text" name="no_kk" value="{{ old('no_kk', $candidate->no_kk) }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Tempat Lahir</label>
                            <input type="text" name="tempat_lahir" value="{{ old('tempat_lahir', $candidate->tempat_lahir) }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Tanggal Lahir</label>
                            <input type="date" name="tanggal_lahir" value="{{ old('tanggal_lahir', $candidate->tanggal_lahir) }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Jenis Kelamin</label>
                            <select name="jenis_kelamin" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                <option value="L" {{ old('jenis_kelamin', $candidate->jenis_kelamin) == 'L' ? 'selected' : '' }}>Laki-laki</option>
                                <option value="P" {{ old('jenis_kelamin', $candidate->jenis_kelamin) == 'P' ? 'selected' : '' }}>Perempuan</option>
                            </select>
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Anak Ke-</label>
                            <input type="number" name="anak_ke" value="{{ old('anak_ke', $candidate->anak_ke) }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        </div>
                        <div>
                ...
                <div class="bg-white shadow-md rounded-xl overflow-hidden mb-6 border border-gray-100">
                    ...
                    <div class="p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-6">
                        ...
                        <div class="lg:col-span-2">
                            ...
                            <input type="text" name="desa" value="{{ old('desa', $candidate->address->desa ?? '') }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                        </div>
                    ...
                    <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-8 divide-y md:divide-y-0 md:divide-x divide-gray-200">
                        ...
                        <div class="space-y-4 pr-0 md:pr-4">
                            ...
                            <div>
                                ...
                                <input type="text" name="nama_ayah" value="{{ old('nama_ayah', $candidate->parent->nama_ayah ?? '') }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                            </div>
                            <div class="grid grid-cols-2 gap-2">
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-1">NIK Ayah</label>
                                    <input type="text" name="nik_ayah" value="{{ old('nik_ayah', $candidate->parent->nik_ayah ?? '') }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-1">Thn Lahir</label>
                                    <input type="number" name="thn_lahir_ayah" value="{{ old('thn_lahir_ayah', $candidate->parent->thn_lahir_ayah ?? '') }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                </div>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Pendidikan</label>
                                @php $pdkA = $candidate->parent->pendidikan_ayah ?? ''; @endphp
                                <select name="pendidikan_ayah" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                    ...
                                    <option value="SD" {{ $pdkA == 'SD' ? 'selected' : '' }}>SD</option>
                                    <option value="SMP" {{ $pdkA == 'SMP' ? 'selected' : '' }}>SMP</option>
                                    <option value="SMA" {{ $pdkA == 'SMA' ? 'selected' : '' }}>SMA</option>
                                    <option value="S1" {{ $pdkA == 'S1' ? 'selected' : '' }}>S1</option>
                                    <option value="S2" {{ $pdkA == 'S2' ? 'selected' : '' }}>S2</option>
                                    <option value="Lainnya" {{ $pdkA == 'Lainnya' ? 'selected' : '' }}>Lainnya</option>
                                </select>
                            </div>
                            <div>
                        ...
                        <div class="space-y-4 pt-4 md:pt-0 md:pl-4">
                            ...
                            <div>
                                ...
                                <input type="text" name="nama_ibu" value="{{ old('nama_ibu', $candidate->parent->nama_ibu ?? '') }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                            </div>
                             <div class="grid grid-cols-2 gap-2">
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-1">NIK Ibu</label>
                                    <input type="text" name="nik_ibu" value="{{ old('nik_ibu', $candidate->parent->nik_ibu ?? '') }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-1">Thn Lahir</label>
                                    <input type="number" name="thn_lahir_ibu" value="{{ old('thn_lahir_ibu', $candidate->parent->thn_lahir_ibu ?? '') }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                </div>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Pendidikan</label>
                                @php $pdkI = $candidate->parent->pendidikan_ibu ?? ''; @endphp
                                <select name="pendidikan_ibu" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                    <option value="">- Pilih -</option>
                                    <option value="SD" {{ $pdkI == 'SD' ? 'selected' : '' }}>SD</option>
                                    <option value="SMP" {{ $pdkI == 'SMP' ? 'selected' : '' }}>SMP</option>
                                    <option value="SMA" {{ $pdkI == 'SMA' ? 'selected' : '' }}>SMA</option>
                                    <option value="S1" {{ $pdkI == 'S1' ? 'selected' : '' }}>S1</option>
                                    <option value="S2" {{ $pdkI == 'S2' ? 'selected' : '' }}>S2</option>
                                    <option value="Lainnya" {{ $pdkI == 'Lainnya' ? 'selected' : '' }}>Lainnya</option>
                                </select>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Pekerjaan</label>
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">No. HP / WA</label>
                                <input type="text" name="no_hp_ibu" value="{{ old('no_hp_ibu', $candidate->parent->no_hp_ibu ?? '') }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            </div>
                        </div>
                    </div>
                </div>

                <div class="fixed bottom-0 left-0 right-0 bg-white border-t p-4 z-50 md:static md:bg-transparent md:border-none md:p-0 md:mb-20">
                    <div class="max-w-7xl mx-auto flex justify-end gap-4">
                        <a href="{{ route('admin.candidates.show', $candidate->id) }}" class="px-6 py-3 bg-white text-gray-700 border border-gray-300 rounded-lg font-bold hover:bg-gray-50 transition shadow-sm">
                            Batal
                        </a>
