...
{{-- SCRIPT AUTO LOGIN DARI QR CODE --}}
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            // Cek apakah ada parameter 'no_daftar' di link URL
            const urlParams = new URLSearchParams(window.location.search);
            const noDaftarParam = urlParams.get('no_daftar');
            
            if (noDaftarParam) {
                // Isi input otomatis dengan nomor dari QR
                const inputElement = document.querySelector('input[name="no_daftar"]');
                if (inputElement) {
                    inputElement.value = noDaftarParam;
                    
                    // Ubah teks tombol biar santri tau sedang diproses otomatis
                    const btn = document.querySelector('button[type="submit"]');
                    if (btn) {
                        btn.innerHTML = '<svg class="animate-spin h-5 w-5 text-white" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg> <span class="ml-2">Mengalihkan Otomatis...</span>';
                    }
                    
                    // Otomatis klik tombol submit
